#ifndef _RPOCOJSON_EXTEND_H_
#define _RPOCOJSON_EXTEND_H_

#include "rpocojson.hpp"
...
namespace rpoco
{
		...
		struct json_parser : public rpoco::visitor {
			...
			virtual void visit(int &iv) {
				...
				if (c == '.' || c == 'e' || c == 'E') {
					...
					tmp = ext::to_string(acc);
					// then consume the rest of the number info
					consume_frac_and_exp();
					if (ok) {
						double dv = ext::stod(tmp);
						iv = (int)dv;
	...
	template<typename X> bool to_cache(std::map<std::string, json_value>, X &x) {
...
namespace rpoco {
	template<> struct visit<rpocojson::json_value> {
		visit(visitor &v, rpocojson::json_value &jv) {
			if (v.peek() == vt_none) {
				switch (jv.type()) {
				case vt_null: {
					v.visit_null();
